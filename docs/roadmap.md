...
> **Статус на 2026-07-07:** фазы 0–4 выполнены (коммиты `d3035bd`, `86a29b1`,
> `6936c47`, `7394ffe`, `fec8c7d`). Открыты: фаза 5 (`buxter pipeline` +
> CADPrompt-бенч — нужен API-ключ для прогона) и фаза 6 (внешние интеграции).
...
| # | Что | Ценность | Статус |
|---|-----|----------|--------|
| 0 | Фиксы ревью | обязательна | ✅ `d3035bd` |
| 1 | Vision-цикл (render + вопросы + kernel-gate) | максимум качества/₽ | ✅ `86a29b1` (matplotlib-рендер; f3d — upgrade-path) |
| 2 | build123d-бэкенд | headless CI-путь | ✅ `6936c47` |
| 3 | Hausdorff-diff + fit-check (manifold3d) | ремонт + сборки | ✅ `7394ffe` (PyMeshFix-repair — отложен) |
| 4 | PrusaSlicer-gate | реальная печатаемость | ✅ `fec8c7d` |
| 5 | pipeline + CADPrompt-bench | воспроизводимость | ⏳ нужен API-ключ для прогона бенча |
| 6 | freecad-mcp / Zoo API / web-сценарии | опционально | ⏳ внешние сервисы |
